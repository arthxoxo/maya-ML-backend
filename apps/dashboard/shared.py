...
def heuristic_sentiment_fallback(text: str) -> tuple[float, float]:
    ...
    # Neutralize pure commands starting with a command verb with no other emotional cues.
    first_tok = tokens[0] if tokens else ""
    if first_tok in COMMAND_VERBS and (score == 0 and phrase_neg == 0 and phrase_pos == 0):
        polarity = polarity * 0.2
